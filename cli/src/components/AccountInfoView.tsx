...
export const AccountInfoView: React.FC<AccountInfoViewProps> = React.memo(({ controller }) => {
	...
	// Show provider info — for openai with ShuttleAI base URL, show ShuttleAI branding
	if (provider !== "cline") {
		const config = StateManager.get().getApiConfiguration() as Record<string, unknown>
		const isShuttleAI = provider === "openai" && config["openAiBaseUrl"] === "https://api.shuttleai.com/v1"
		const displayName = isShuttleAI ? "ShuttleAI" : capitalize(provider || "Not configured")
		return (
			<Box>
	...
	if (!email) {
		return (
			<Box>
				...
				<Text color="yellow">Not logged in (run 'shuttle auth' to sign in)</Text>
			</Box>
		)
	}
