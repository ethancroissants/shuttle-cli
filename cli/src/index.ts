...
async function runTaskInPlainTextMode(
	...
	if (!hasAuth) {
		printWarning("Not authenticated. Please run 'shuttle auth' first to configure your ShuttleAI API key.")
...
async function initializeCli(options: InitOptions): Promise<CliContext> {
	...
	const outputChannel = window.createOutputChannel("ShuttleAI CLI")
	...
	outputChannel.appendLine(
		`ShuttleAI CLI initialized. Data dir: ${DATA_DIR}, Extension dir: ${EXTENSION_DIR}, Log dir: ${CLINE_CLI_DIR.log}`,
	)
...
program.name("shuttle").description("ShuttleAI CLI - AI coding assistant in your terminal").version(CLI_VERSION)
...
program
	...
	.option("--config <path>", "Path to ShuttleAI configuration directory")
...
program
	.command("version")
	.description("Show ShuttleAI CLI version number")
	.action(() => printInfo(`ShuttleAI CLI version: ${CLI_VERSION}`))
...
async function resumeTask(taskId: string, options: TaskOptions & { initialPrompt?: string }) {
	...
	if (!historyItem) {
		...
		printInfo("Use 'shuttle history' to see available tasks.")
		await disposeCliContext(ctx)
		exit(1)
	}
