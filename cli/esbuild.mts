...
const copyWasmFiles: esbuild.Plugin = {
	...
	setup(build) {
		build.onEnd(() => {
			// Skip WASM copying when --no-wasm flag is passed (faster dev builds)
			if (skipWasm) return
...
const sharedOptions: Partial<esbuild.BuildOptions> = {
	...
	platform: "node",
	target: "node20",
	// These modules load files from their own directory at runtime or are large — keep external
	external: [
		...
		"better-sqlite3",
		"@vscode/ripgrep",
		// React/Ink (large, already in node_modules)
		"ink",
		"ink-spinner",
		"ink-picture",
		"react",
		...
		"pino-roll",
		"aws4fetch",
		// AI provider SDKs — large, keep external so bundle stays small & builds are fast
		"@anthropic-ai/sdk",
...
async function main() {
	if (watch) {
		...
		await ctx.watch()
		console.log("[shuttle] Watching for changes...")
	} else {
		console.log("[shuttle] Building CLI executable...")
		...
		console.log("[shuttle] Done → dist/cli.mjs")
	}
}
